...
public:
    ...
    void sendCall(const string& callee) override {
		printTradeMark("Phone");
        cout << ": made a call to " << callee << endl;
        baseStation->connectTo(owner, callee);
    }

    void receiveCall(const string& caller) override {
		printTradeMark("Phone");
        cout << ": received a call from " << caller << endl;
    }
...
SmartPhone* Person::newSmartPhone() {
	/*return (id % 2 != 0) ?
			static_cast<SmartPhone*>(new GalaxyPhone(name)) : static_cast<SmartPhone*>(new IPhone(name, "13"));*/
	if(id % 2 != 0)
...
void Person::setSmartPhone(SmartPhone* smPhone) {
	if(smartPhone != nullptr)
		delete smartPhone;
	(smPhone == nullptr) ? smartPhone = newSmartPhone() : smartPhone = smPhone;
}
...
void PersonManager::run() {
    ...
    func_t func_arr[] = {
		...
		&PM::insert, &PM::remove, &PM::copyPersons, &PM::reset, &PM::find,
		&PM::dispStudentWorkers, &PM::dispPhones,
    };
